            ...
            <div class="bg-white pt-2 pl-3 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div>
                    @if ($article->user->id === Auth::user()->id)
                        ...
                        <h3 class="text-center font-semibold text-xl text-gray-800 leading-tight">{{ $article->title }}
                        </h3>
                        <div
                        ...
                        <div>
                            <a class="inline-flex items-center">
                                <img alt="blog" src="https://dummyimage.com/104x104"
                                    class="w-12 h-12 rounded-full flex-shrink-0 object-cover object-center">
                                <span class="flex-grow flex flex-col pl-4">
                                    <span class="title-font font-medium text-gray-900"><a class="text-blue-500"
                                            href="{{ route('profile.show', ['id' => $article->user->id]) }}">{{ $article->user->name }}</a></span>
                                    <span
                                        class="text-gray-400 text-xs tracking-widest mt-0.5">{{ $article->created_at->format('Y-m-d') }}</span><span>【{{ $article->user->department->name }}】</span>
                                </span>
                            </a>
                ...
                <favorite-button>
                    @if (Auth::check())
                        @if ($article->isFavoritedByUser(Auth::user()))
                            <form action="{{ route('articles.unfavorite', ['article' => $article->id]) }}"
                        ...
                        @else
                            <form action="{{ route('articles.favorite', ['article' => $article->id]) }}" method="POST"
                                class="mb-4">
                                @csrf
                                <input type="hidden" name="article_id" value="{{ $article->id }}">
                                <button type="submit">
                ...
                <div class="pull-right article-user-link">
                    @if ($article->user_id === Auth::user()->id)
                        <li><a class="bg-primary" href="{{ route('articles.myblog', Auth::user()->id) }}">マイブログへ</a>
                        </li>
                    @else
                        <a
                            href="{{ route('articles.myblog', ['id' => $article->user_id]) }}"><span>{{ $article->user->name }}</span>さんのブログ一覧へ</a>
                    @endif
                </div>
                        ...
                        <div class="flex-start md:flex">
                            ...
                            <div
                                ...
                                <div class="mb-4 flex justify-between">
                                    <a href="#!"
                                        class="text-sm text-info transition duration-150 ease-in-out hover:text-info-600 focus:text-info-600 active:text-info-700">回答者の名前</a>さんが回答
                                    <a href="#!"
                                        class="text-sm text-info transition duration-150 ease-in-out hover:text-info-600 focus:text-info-600 active:text-info-700">04
                                        / 02 / 2022（回答日の日付）</a>
                                </div>
                                ...
                                <button type="button"
                                    class="inline-block rounded mb-2 rounded px-6 py-2.5 text-xs font-medium uppercase leading-normal text-white shadow-md transition duration-150 ease-in-out hover:shadow-lg focus:shadow-lg focus:outline-none focus:ring-0 active:shadow-lg"
                                    style="background-color:rgb(0, 162, 255)" data-te-ripple-init
                                    data-te-ripple-color="light">
                                    リプライ
                                </button>
                            </div>
